...
use crate::cluster::{BuildCount, Cluster, ClusterBuild, RadixTrie};
...
const FILTER_BUILDINGS: [&str; 3] = ["Assimilator", "Refinery", "Extractor"];
...
impl Builds {
    ...
    pub fn generate_matchup_build_trees(&mut self) {
        ...
        println!(
            "total time spend generating matchup build trees: {:.2?}",
            finish
        );
        ...
        loop {
            ...
            for (build, other_build, _) in &optimal_cluster_comparisons {
                ...
                for clustered_build in &self.build_clusters[build].cluster {
                    ...
                    let cross_cluster_diff =
                        &self.build_comparison_information[&build_comparison_identifier];
                    if *cross_cluster_diff > MAX_COMPARISON_DIFF {
            ...
            if completed {
                ...
                for cluster in self.build_clusters.values_mut() {
                    ...
                    cluster.cluster.sort_by(|a, b| {
                        a.diff
                            .partial_cmp(&b.diff)
                            .expect("build diff should be a float")
                    });

                    let deconstructed_root_build: Vec<&str> =
                        cluster.build.build.split(SECTION_SEPARATOR).collect();
                    ...
                    for (idx, build) in cluster.cluster.iter().enumerate() {
                        ...
                        let deconstructed_build: Vec<&str> =
                            build.build.split(SECTION_SEPARATOR).collect();
                ...
                builds.sort_by(|a, b| b.build.total.cmp(&a.build.total));
