...
const WORKERS: [&str; 3] = ["SCV", "Probe", "Drone"];
    ...
    pub fn new(
        ...
        for EventEntry(field, value) in &event.entries {
            match field.as_str() {
                ...
                "m_unitTypeName" => {
                    if let DecoderResult::Blob(name) = value {
                ...
                "m_unitTagIndex" => {
                    if let DecoderResult::Value(index) = value {
                        tag_index = *index as u32;
                    }
                }
                "m_unitTagRecycle" => {
                    if let DecoderResult::Value(recycle) = value {
                        tag_recycle = *recycle as u32;
                    }
                }
                "_gameloop" => {
                    if let DecoderResult::Value(gameloop) = value {
        ...
        if calculated_gameloop > 0 && calculated_gameloop < 9408 {
            ...
            {
                game.builds[player_index as usize]
                    .push((game_object_name.to_owned(), calculated_gameloop));
            }
