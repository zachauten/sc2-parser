...
impl<'a> BitPackedDecoder<'a> {
    pub fn new(contents: Vec<u8>, typeinfos: &'a [ProtocolTypeInfo<'a>]) -> BitPackedDecoder<'a> {
...
impl<'a> VersionedDecoder<'a> {
    pub fn new(contents: Vec<u8>, typeinfos: &'a [ProtocolTypeInfo<'a>]) -> VersionedDecoder<'a> {
        let buffer = BitPackedBuffer::new(contents);
